  ...
  return (
    <section className="product-showcase">
      <div className="showcase-header">
        <h2 className="showcase-title">{locale === 'en' ? 'NEW ARRIVALS' : '新作コレクション'}</h2>
        <Link href="/type/plant" className="view-all-link hover-lift">
      ...
      <div className="showcase-grid">
        {products.map((product) => {
          const kindStr = String(product.kind || '').toLowerCase();
          const isPlant = kindStr.includes('plant');
          const isFurniture = kindStr.includes('furniture');
          const elementStr = Array.isArray(product.element) ? product.element[0] : (product.element || '');
            ...
            <Link href={`/product/${product.id}`} key={product.id} className="showcase-card">
              ...
              <div className="sc-info">
                <span className="sc-category">
                  {isFurniture ? "FURNITURE" : isPlant ? "HOUSEPLANTS" : elementStr.toUpperCase()}
                </span>
