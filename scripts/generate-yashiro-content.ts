...
const importPlants = async () => {
  const dataPath = path.resolve(process.cwd(), 'scripts/new-plants-data.json');
  ...
  console.log(`🚀 Starting import of ${plants.length} pre-generated plants to microCMS...`);
  let successCount = 0;
  ...
  for (const plant of plants) {
    try {
      const productPayload = {
        ...
        price_m: plant.price_m,
        price_l: plant.price_l
      };
